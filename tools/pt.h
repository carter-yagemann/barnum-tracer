...
static void add_page(unsigned long base, unsigned long size)
{
    mapped_page *page = (mapped_page *) malloc(sizeof(mapped_page));
    page->base = base;
    page->size = size;
    LL_APPEND(mapped_pages, page);
}

static char addr_is_mapped(unsigned long addr)
{
    mapped_page *page;
    LL_FOREACH(mapped_pages, page)
    {
        if (page->base <= addr && page->base + page->size > addr)
            return 1;
    }
...
retry:
    ...
    switch (META_GET_FC(inst.meta))
    {
        ...
        case FC_INT:
            ...
            break;
        case FC_CMOV:
            codeInfo.code = (unsigned char *) pt_ip_to_code(codeInfo.nextOffset);
...
static inline char pt_in_block(unsigned long a, pt_block *b)
{
    pt_block *res = pt_get_block(a);

    if (!res)
        return 0;

    return res->fallthrough_addr == b->fallthrough_addr;
}
...
static inline void
pt_recover(unsigned char *buffer, unsigned int size, pt_recover_arg arg)
{
...
#define FOLLOW_DIRECT_UNTIL(cond) \
    do { \
        while (curr_block && pt_block_is_direct(curr_block) && (!(cond))) { \
            ...
            if (addr_is_mapped(pt_get_target_addr(curr_block))) { \
        ...
        switch (kind)
        {
            case PT_PACKET_TNTSHORT:
                ...
                do
                {
                    ...
                    {
#ifdef PT_H_DEBUGGING
                        printf("WARNING: Follow direct led to unmapped code\n");
#endif
                        return;
                    }

                    if (!pt_block_is_cond(curr_block) && !pt_block_is_ret(curr_block))
                    {
#ifdef PT_H_DEBUGGING
                        printf("WARNING: Block is not conditional or return, but we're in TNT\n");
#endif
                        return;
                    }
                    ...
                    if (mask & (bit_selector >>= 1))
                    {
                        ...
                        if (pt_block_is_ret(curr_block))
                        {
                            ...
                            {
#ifdef PT_H_DEBUGGING
                                printf("WARNING: Return without call\n");
#endif
                                return;
                            }
                    ...
                    else
                    {
                        ...
                        {
#ifdef PT_H_DEBUGGING
                            printf("WARNING: Block is a return, but we're in a TNT not taken\n");
#endif
                            return;
                        }
            ...
            case PT_PACKET_TIP:
                ...
                {
#ifdef PT_H_DEBUGGING
                    printf("WARNING: TIP to unmapped address %lx\n", curr_addr);
#endif
                    return;
                }
                break;
            ...
            case PT_PACKET_TIPPGE:
                ...
                {
#ifdef PT_H_DEBUGGING
                    printf("WARNING: PT turned on at an address that isn't mapped (%lx)\n", curr_addr);
#endif
                    return;
                }
                pt_on_block(curr_addr, arg);
